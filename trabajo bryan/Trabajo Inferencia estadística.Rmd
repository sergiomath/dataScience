...
```
Es posible observar que las varianzas de los grupos son homogeneas , sin embargo existen varios valores atípicos para algunos grupo que podrían cambiar el valor del test .

```{r}
regreAnova=lm(Base$`Promedio Primer Semestre`~factor(Base$`Estrato social`))
anova=aov(regreAnova)
summary(anova)
```
...
los estudiantes que entran  a facultad de psicología , sin embargo en la busqueda a la respuesta de este interrogante encontramos una relación entre el número de horas dedicacas al estudio y el incremento dela nota promedio de los estudiantes . Además , se encontró un modelo para poder intentar precedicir con una acertividad del 70% aproximadamente.
